...
begin
    if mode == :zoo
        ...
        md"""
        Tool Version: $(@bind zoo_pick_version Select(versions; default="2023x"))
        """
    else
        nothing
    end
...
begin
    ...
    def_start = FMI.getDefaultStartTime(descr)
    if isnothing(def_start)
        def_start = 0.0
    end

    def_stop = FMI.getDefaultStopTime(descr)
...
if length(param_vrs) > 0
    @bind changeParameters MultiSelect(
        collect(
            param_vrs[i] => FMI.valueReferenceToString(fmu, param_vrs[i])[1] for
...
## Record values
### States 
$(@bind recordStates MultiSelect(collect(state_vrs[i] => FMI.valueReferenceToString(fmu, state_vrs[i])[1] for i in 1:length(state_vrs))))

### Derivatives 
$(@bind recordDerivatives MultiSelect(collect(ders_vrs[i] => FMI.valueReferenceToString(fmu, ders_vrs[i])[1] for i in 1:length(ders_vrs))))

### Inputs 
$(@bind recordInputs MultiSelect(collect(input_vrs[i] => FMI.valueReferenceToString(fmu, input_vrs[i])[1] for i in 1:length(input_vrs))))

### Outputs 
$(@bind recordOutputs MultiSelect(collect(output_vrs[i] => FMI.valueReferenceToString(fmu, output_vrs[i])[1] for i in 1:length(output_vrs))))

### Parameters 
$(@bind recordParameters MultiSelect(collect(param_vrs[i] => FMI.valueReferenceToString(fmu, param_vrs[i])[1] for i in 1:length(param_vrs))))

### Internal values 
$(@bind recordInternals MultiSelect(collect(internal_vrs[i] => FMI.valueReferenceToString(fmu, internal_vrs[i])[1] * " [" * string(internal_vrs[i]) * "]" for i in 1:length(internal_vrs))))
"""
...
md"""
x-axis: $(@bind xaxis Select([0 => "time", collect(recordValues[i] => FMI.valueReferenceToString(fmu, recordValues[i])[1] for i in 1:length(recordValues))...]))
"""
        ...
        for i = 1:length(recordValues)
            ...
            if xaxis != rv
                ...
                plot!(
                    ...
                    xaxis_vals,
                    vals;
                    label = FMI.valueReferenceToString(fmu, rv)[1] *
                            " [" *
                            string(rv) *
                            "]",
                )
...
manifest_format = "2.0"
project_hash = "47ea6f5a9e106a1f96410403b383b19a4b96294a"
