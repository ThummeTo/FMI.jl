...
solution = fmiSimulateME(fmuStruct, t_start, t_stop; inputValueReferences=["extForce"], inputFunction=extForce_t, solver=Rosenbrock23(autodiff=true), dtmax=0.001) # dtmax to force resolution
@test length(solution.states.u) > 0
@test length(solution.states.t) > 0
